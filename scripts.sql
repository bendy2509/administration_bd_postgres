...
-- 2. SUPPRESSION : Supprimer les affectations d'un professeur inactif
DELETE
FROM affectations_professeurs
WHERE professeur_id IN (SELECT id
                        FROM professeurs
                        WHERE actif = false)
RETURNING id, professeur_id, matiere_id;

-- =============================================
-- REQUETES ANALYTIQUES AVANCEES
-- =============================================

-- 1. Taux d'occupation des salles par heure
WITH creneaux_etendus AS (SELECT s.id  AS salle_id,
                                 s.numero,
                                 b.nom AS batiment,
                                 generate_series(
                                         date_trunc('hour', ch.heure_debut),
                                         date_trunc('hour', ch.heure_fin) - INTERVAL '1 hour',
                                         INTERVAL '1 hour'
                                 )     AS heure
                          FROM creneaux_horaires ch
                                   JOIN salles s ON ch.salle_id = s.id
...
SELECT batiment,
       ...
       ROUND(COUNT(*) * 100.0 / (SELECT COUNT(DISTINCT DATE(heure)) FROM creneaux_etendus), 1) AS taux_occupation
FROM creneaux_etendus
GROUP BY batiment, numero, EXTRACT(HOUR FROM heure)
ORDER BY batiment, numero, heure;

-- 3. Matières avec le plus de créneaux
SELECT m.code_matiere,
       m.nom                                                                     AS matiere,
       pr.nom                                                                    AS programme,
       COUNT(ch.id)                                                              AS nombre_creneaux,
       COUNT(DISTINCT ch.professeur_id)                                          AS nombre_professeurs,
       ROUND(AVG(EXTRACT(EPOCH FROM (ch.heure_fin - ch.heure_debut)) / 3600), 1) AS duree_moyenne_heure
FROM matieres m
         JOIN programmes pr ON m.programmes_id = pr.id
         LEFT JOIN creneaux_horaires ch ON m.id = ch.matiere_id
    AND ch.statut IN ('planifie', 'confirme')
GROUP BY m.id, m.code_matiere, m.nom, pr.nom
ORDER BY nombre_creneaux DESC
LIMIT 10;

-- 4. Évolution de l'emploi du temps (comparaison semestre 1 vs semestre 2)
WITH stats_semestre AS (SELECT m.semestre,
                               COUNT(DISTINCT ch.id)            AS total_creneaux,
                               COUNT(DISTINCT ch.professeur_id) AS nombre_professeurs,
                               COUNT(DISTINCT ch.matiere_id)    AS nombre_matieres,
                               ROUND(AVG(EXTRACT(EPOCH FROM (ch.heure_fin - ch.heure_debut)) / 3600),
                                     2)                         AS duree_moyenne
                        FROM creneaux_horaires ch
                                 JOIN matieres m ON ch.matiere_id = m.id
                        WHERE ch.statut IN ('planifie', 'confirme')
                          AND m.annee_academique = '2024-2025'
                        GROUP BY m.semestre)
SELECT semestre,
       total_creneaux,
       nombre_professeurs,
       nombre_matieres,
       duree_moyenne,
       ROUND((total_creneaux - LAG(total_creneaux) OVER (ORDER BY semestre)) * 100.0 /
             LAG(total_creneaux) OVER (ORDER BY semestre), 1) AS evolution_creneaux_pourcent
FROM stats_semestre
ORDER BY semestre;
